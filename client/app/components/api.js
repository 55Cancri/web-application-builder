...
STEP 3 (redux): The axios request below may look difficult (even though it is short), but it is actually very simple. We now have the credentials that passed from the previous function (api.user.login(credentials)), and put that form data ({email: "", password: ""}) in the login function as a parameter.
...
After this, a promise is returned in the form a response. We assign the response to the variable "res" and get the data.user property from the res object. We will have the email and the token. And, then there are still unfullfilled promises in reducer and and LoginPage components, so we return BACK to the reducer.js file.
